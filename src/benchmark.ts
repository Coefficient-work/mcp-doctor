...
export type BenchmarkRunResult = {
  rows: BenchmarkRow[];
  reports: Array<{ id: string; markdown: string }>;
};

export function loadBenchmarkCatalog(path?: string): BenchmarkEntry[] {
  const dir = dirname(fileURLToPath(import.meta.url));
  const catalogPath = path ?? join(dir, "../examples/benchmark-catalog.json");
  const raw = JSON.parse(readFileSync(resolve(catalogPath), "utf8")) as {
    servers: BenchmarkEntry[];
  };
  return raw.servers;
}
...
export async function runBenchmark(
  ...
  const reports: Array<{ id: string; markdown: string }> = [];
...
export function formatStateOfMcpReport(rows: BenchmarkRow[], date = new Date().toISOString().slice(0, 10)): string {
  ...
  if (ok.length > 0) {
    ...
    lines.push(
      ...
      `- **Most tools (bloat risk):** ${mostTools?.name} (${mostTools?.toolCount} tools)`,
      `- **Worst tool bloat:** ${worstBloat?.name} (${worstBloat?.tokens} tokens)`,
    );
  }
  ...
  lines.push(
    "",
    "---",
    "_v0 benchmark uses static agent-readiness checks. v1 will add BYOK task evals + friction scores._",
  );
