...
const DESTRUCTIVE_NAME_RE = /delete|remove|destroy|purg(e|ing|ed)|drop|cancel|nuke|flush|wipe|kill|terminate|reset|revoke|truncate|zeroing|zeroed|^zero_|_zero_|\bzero\b|overwrit|prun(e|ing)|reboot/i;
...
const SECRET_VALUE_NAME_RE = /^(api[_-]?key|secret_api_key|signing_secret|pd_token|password|passwd|secret|credential)$|(^|_)(api[_-]?key|api[_-]?token|api_secret|password|passwd|signing_secret)$|(^|_)(secret|password)$|(^|_)token$/i;
const SECRET_REFERENCE_NAME_RE = /(^|_)(ref|pointer)$|(^|_)vault(_|$)|^vault_/i;
const SECRET_VALUE_DESC_RE = /\bbearer token\b|\bhmac secret\b|\bapi key to pass\b|\bauth token\b|\bthe (actual )?(api )?key itself\b/i;
...
export function runScorecard(doc, tools = operationsFromDoc(doc), options = {}) {
    ...
    if (tools.length === 0) {
        ...
        const blocksGradeA = checks.some((check) => GRADE_A_BLOCKING_CHECKS.has(check.id) &&
...
function checkDuplicateNames(tools) {
    ...
    const ambiguous = tools.filter((t) => t.name.length < 4 || /^[a-z]$/.test(t.name) || VAGUE_TOOL_NAME_RE.test(t.name));
    const results = [];
    if (dupes.length > 0) {
        results.push({
            id: "duplicate-names",
...
function checkDescriptions(tools) {
    ...
    if (thin.length > 0 || bloated.length > 0) {
        const bits = [];
        if (thin.length > 0)
            bits.push(`${thin.length} thin (<30 chars)`);
        if (bloated.length > 0)
            bits.push(`${bloated.length} bloated (>${DESCRIPTION_BLOATED_CHARS} chars)`);
        return [{
                id: "descriptions",
                category: "docs",
                severity: "warn",
...
function checkMissingRequired(tools) {
    const invalid = tools.filter((t) => {
        ...
        if (!Array.isArray(required))
            return true;
        return required.some((name) => typeof name !== "string" || !Object.prototype.hasOwnProperty.call(props, name));
    });
    if (invalid.length > 0) {
        return [{
                id: "missing-required",
...
function isDestructivelyMarked(tool) {
    if (DESTRUCTIVE_MARKED_RE.test(tool.description))
        return true;
